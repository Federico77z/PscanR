...
such as the score, position and oligonucleotide seuqence for each hit within the
background, to reduce computational time.

When you export the PSMatrixList with `ps_write_bg_to_file()` this infromation
gets discarded to conserve memory. In fact, after the computation of the
background mean and standard deviation, these data are no more needed since can
be efficiently recomputed by `pscan()`.
...
## Mega-background Generation

To create a mega-background dataset, you need to store the full output of the
`ps_build_bg()` function without discarding any computed values. This can be
fullfilled using the `save()` function in R and by setting the megaBG flag on
TRUE. This will generate an .RData file containing the hits score, position,
strand, and oligonucleotide sequence, for each sequence scanned with the PWMs.

## Application

Once the mega-background has been created, it can be used in subsequent analyses
to streamline computations. The `pscan_bg()` function requires two inputs:

1.  **A vector of transcript names** -- This vector should contain the
    identifiers of the transcripts of interest.

2.  **The generated mega-background dataset** -- The `.RData` file created in
    the previous step.

By using `pscan_bg()` with the saved mega-background, the analysis process
