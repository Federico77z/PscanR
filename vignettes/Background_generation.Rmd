...
output:
    BiocStyle::html_document:
        ...
        toc_depth: 2
vignette: >
    %\VignetteIndexEntry{3. Generate a PscanR Background Model}
...
BiocManager::install(c(
    "txdbmaker", "GenomicFeatures", "GenomeInfoDb",
...
```{r retrieve-jaspar-2022, eval=FALSE}
opts <- list()
...
To keep this vignette reproducible without accessing an external data cache,
the executable example uses two matrices from the JASPAR2020 collection bundled
with PscanR:

```{r local-jaspar-matrices}
matrix_path <- system.file("extdata", "J2020.rds", package = "PscanR")
example_matrices <- readRDS(matrix_path)[seq_len(2)]
example_matrices
```
...
```{r retrieve-jaspar-2024, eval=FALSE}
...
**Warning**: Using **a single core** (`BPPARAM = BiocParallel::SerialParam()`)
...
```{r echo=TRUE}
example_PSBG <- PscanR::ps_build_bg(
    prom_seq[1:10],
    example_matrices,
...
PscanR::ps_write_bg_to_file(
    example_PSBG,
    "example_hg38_200u_50d.psbg.txt"
)
```
...
analyses  to streamline computations. The `pscan_fullBG()` function requires 
two inputs:
...
By using `pscan_fullBG()` with the saved full-background, the analysis process
