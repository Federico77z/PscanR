...
RefSeq identifiers, for Arabidopsis on TAIR, and for yeast on SGD IDs. 
Other types of gene identifiers can also be used if the corresponding background 
model is created (see the vignette "How to Generate a Background Model for the 
PscanR Algorithm" for more details).

A function is also available that accepts only gene identifiers as input 
(see the pscan_full_bg documentation for further information).

Below is an example dataset containing **human NF-κB target genes**, 
curated from the literature. The number **50** indicates that 50% genes in the 
list are known NF-κB targets.
