...
generation, the second is the core function of **PscanR**. This vignette focuses
on analyzing how varying the number of cores passed to the `bpparam()` function
(one of `pscan()`'s inputs) affects execution time.
...
and **50 bp downstream** from the Transcription Start Site (**TSS**). The number
of cores tested includes: **1, 2, 4, 6, 8, 12, 18, and 24**.
...
# Retrieve target sequence identifiers. You can download it from the official Pscan platform
...
execution_time <- function(prom_seq, bg, ncores, output_file){
  ...
  for (i in 1:iterations){
    ...
    if (i == 1){
      saveRDS(PscanR::ps_results_table(results), output_file) # Analyse all the files to be sure that results don't change when changing the core numbers
      }
...
```{r}
file_path <- system.file("extdata", "df_processori_SnowParam.rds", package = "PscanR")
data <- readRDS(file_path)
...
This vignette demonstrated the impact of parallelization on **PscanR** execution
time. The results show that **MulticoreParam** is more efficient but limited to
Unix systems, while **SnowParam** ensures compatibility across all platforms. A
plateau in performance gain beyond **8 cores** suggests an optimal configuration
