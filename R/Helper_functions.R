...
.download_background <- function(file, destfile = NULL) {
    ...
    "dianabetelli/PscanR_backgrounds/refs/heads/main/BG_files/",
    file
    )
...
.download_available_backgrounds <- function(destfile = NULL) {
    ...
    src <- system.file("extdata", "AvailableBG.txt", package = "PscanR")
    if (src == "") {
    stop("AvailableBG.txt not found in installed package data.")
    }

    ok <- file.copy(src, destfile, overwrite = TRUE)
    if (!ok) {
    stop(sprintf("Failed to copy AvailableBG.txt to %s", destfile))
    }
